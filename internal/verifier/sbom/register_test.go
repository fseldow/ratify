...
func TestNewVerifier(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid with disallowed licenses and packages",
			opts: verifier.NewOptions{
				Name: "sbom",
				Type: verifierTypeSBOM,
				Parameters: map[string]any{
					...
					"disallowedPackages": []map[string]any{{"name": "zlib", "version": "1.2.13-r0"}},
				},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if v.Type() != verifierTypeSBOM {
				t.Fatalf("Type() = %s, want %s", v.Type(), verifierTypeSBOM)
			}
		})
	}
}

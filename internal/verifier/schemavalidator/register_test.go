...
func TestNewVerifier(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			opts: verifier.NewOptions{
				Name:       "sv",
				Type:       verifierTypeSchemaValidator,
		...
		},
		{
			...
			opts: verifier.NewOptions{
				Name:       "sv",
				Type:       verifierTypeSchemaValidator,
				Parameters: map[string]any{"artifactTypes": "application/sarif+json"},
			},
			wantErr:     true,
			errContains: "at least one schema is required",
		},
		{
			name: "valid",
			opts: verifier.NewOptions{
				...
				Parameters: map[string]any{
					...
					"schemas": map[string]string{
						"application/sarif+json": "file:///schemas/sarif-2.1.0.json",
					},
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if v.Type() != verifierTypeSchemaValidator {
				t.Fatalf("Type() = %s, want %s", v.Type(), verifierTypeSchemaValidator)
			}
		})
	}
}

...
@test "validate ratify/gatekeeper tls cert rotation" {
    ...
    run bash -c "kubectl get Provider ratify-gatekeeper-mutation-provider -o json | jq --arg ca \"$(cat .staging/rotation/ca.crt | base64)\" '.spec.caBundle=\$ca' | kubectl replace -f -"
    run bash -c "kubectl get Provider ratify-gatekeeper-provider -o json | jq --arg ca \"$(cat .staging/rotation/ca.crt | base64)\" '.spec.caBundle=\$ca' | kubectl replace -f -"

    # update the ratify tls secret to use the new tls cert and key
    run bash -c "kubectl get secret ratify-gatekeeper-provider-tls -n ${RATIFY_NAMESPACE} -o json | jq --arg cert \"$(cat .staging/rotation/server.crt | base64)\" --arg key \"$(cat .staging/rotation/server.key | base64)\" '.data[\"tls.key\"]=\$key | .data[\"tls.crt\"]=\$cert' | kubectl replace -f -"

    # update the gatekeeper webhook server tls secret to use the new cert bundle
    run bash -c "kubectl get Secret gatekeeper-webhook-server-cert -n ${RATIFY_NAMESPACE} -o json | jq --arg caCert \"$(cat .staging/rotation/gatekeeper/ca.crt | base64)\" --arg caKey \"$(cat .staging/rotation/gatekeeper/ca.key | base64)\" --arg tlsCert \"$(cat .staging/rotation/gatekeeper/server.crt | base64)\" --arg tlsKey \"$(cat .staging/rotation/gatekeeper/server.key | base64)\" '.data[\"ca.crt\"]=\$caCert | .data[\"ca.key\"]=\$caKey | .data[\"tls.crt\"]=\$tlsCert | .data[\"tls.key\"]=\$tlsKey' | kubectl replace -f -"
